...
// When a round is over then is needed to return guessesLeft and guessesSoFar to their original values, and create a new random letter.
// And, because I don't want to repeat it twice (in a case of a win and in a case of a losse), I am going to create a function to reset:
function reset() {
      ...
      compuLetter = possibleCompuLetters [Math.floor(Math.random()*possibleCompuLetters.length)];            
...
var compuLetter = "";
compuLetter = possibleCompuLetters [Math.floor(Math.random()*possibleCompuLetters.length)];
console.log("compuLetterGlobal " + compuLetter); 
        
                          // Waiting for the user to pick a letter
//  This function is run whenever the user presses a key.
   document.onkeyup = function(event) {
      console.log("compuLetterInsideonkeyup " + compuLetter);
      ...
      // if the code gets here is because the user lost a chance to guess no yet lost the round
      if ((userGuess !== compuLetter) && (round.guessesLeft>0))  {
